...
| 2026-04-18T08:15:14Z | staging (Render free, post S1/S2/S3/S4 fix) | staging-hall-1 | rehearsal — step 3 (TV-display subscribe + draw mirror) — re-run post-S4 | agent-2 | 08:15:14Z | 08:15:20Z | pass | `POST /api/admin/halls/hall-default/display-tokens` ack OK — `app_hall_display_tokens` table now present. TV socket `admin-display:login` ack `hallId=hall-default`, `admin-display:subscribe` ack OK. Host drove room `BINGO1` + 3 draws, TV received 2 `draw:new` broadcasts (third draw throttled by BIN-253 rate-limit, not relevant to the mirror-invariant). Token revoked post-test. Varighet ~6 s. Harness note: TV-subscribe now sequenced **after** `room:create` — BIN-498's `admin-display:subscribe` resolves the active hall room at subscribe-time, so subscribing before a game exists gives the hall-display channel but not the per-room event stream. [BIN-498](https://linear.app/bingosystem/issue/BIN-498) / [BIN-503](https://linear.app/bingosystem/issue/BIN-503). |
...
### Findings — S4: staging `app_hall_display_tokens` table missing (2026-04-18 re-run) — ✅ resolved

**Resolution** (2026-04-18, slot-1 ran `npm --prefix apps/backend run migrate` on Render shell). Step 3 re-run at 08:15 passed: display-token creation OK, `admin-display:login`+`subscribe` OK, TV received live `draw:new` broadcasts.

**Harness learning captured in-repo:** `admin-display:subscribe` resolves the active hall-room at subscribe time (BIN-498 design), so a TV kiosk that boots before the first game of the evening will only be in the hall-display broadcast channel, not the per-room channel. Re-subscribing after the hall hosts a game picks up the room. This is a design behavior, not a bug — but documented here so operators don't debug "no draws on TV" when the fix is "reload the TV page after the host starts the game". Future follow-up (non-blocking) could make `admin-display:subscribe` auto-join any new room in the hall; not needed for pilot F1.

...
- ~~Unity archive available on CDN~~ ([BIN-532](https://linear.app/bingosystem/issue/BIN-532) — **canceled 2026-04-17**). Unity permanent decommissioned. `legacy/unity-client/` retained in repo as read-only code reference only — no archive built, no CDN bundle, no `UNITY_LICENSE`. Rollback strategy is git-revert of the web-client deploy per [`PILOT_CUTOVER_RUNBOOK.md`](../operations/PILOT_CUTOVER_RUNBOOK.md) §3. No pre-pilot Unity check needed.

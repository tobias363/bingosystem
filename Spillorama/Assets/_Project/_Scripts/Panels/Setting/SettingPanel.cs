...
public class SettingPanel : MonoBehaviour
{
    ...
    public void setDataOpen()
    {
        ...
        EventManager.Instance.PlayerSettings((socket, packet, args) =>
       {
           Debug.Log($"PlayerSettings Response : {packet}");

           EventResponse<SettingResult> resp = JsonUtility.FromJson<EventResponse<SettingResult>>(
    ...
    public void availBlockGames()
    {
        EventManager.Instance.PlayerSettings((socket, packet, args) =>
       {
           ...
           if (resp.status == "success")
           {
               AvailableblockGamePopup.SetDataAndOpen(resp.result.availableBlockOptions);
           }
           else
           {
               UIManager.Instance.messagePopup.DisplayMessagePopup(resp.message);
    ...
    public void existBlockGames()
    {
        EventManager.Instance.PlayerSettings((socket, packet, args) =>
       {
           ...
           if (resp.status == "success")
           {
               ExistblockGamePopup.SetDataOpen(resp.result.existingBlockRules);
           }
           else
           {
               UIManager.Instance.messagePopup.DisplayMessagePopup(resp.message);
    ...
    public void OnToggleSoundStatusTap(bool isOn)
    {
        ...
        EventManager.Instance.PlayerSoundAndVoiceSettings("sound", Rand, (socket, packet, args) =>
        {
            Debug.Log($"PlayerSoundAndVoiceSettings Response : {packet}");
            ...
            if (resp.status == "success")
            {
                ...
                //SoundManager.Instance.SoundStatus = toggleSoundStatus.isOn;
    ...
    {
        //SoundManager.Instance.voiceStatus = toggleVoiceStatus.isOn;
        int Rand;
        if (toggleVoiceStatus.isOn)
        {
            Rand = 1;
        }
        else
        ...
        EventManager.Instance.PlayerSoundAndVoiceSettings("voice", Rand, (socket, packet, args) =>
          {
              Debug.Log($"PlayerSoundAndVoiceSettings Response : {packet}");
    ...
    public void nextButtonTap(bool isNextTap)
    {
        ...
        if (isNextTap)
        {
            if (CurrentID.Equals(0))
            {
                ...
                int Rand = PlayerPrefs.GetInt(LanguagePrefKey, 2);
                EventManager.Instance.PlayerSoundAndVoiceSettings("voiceLanguage", Rand, (socket, packet, args) =>
                {
                    Debug.Log($"PlayerSoundAndVoiceSettings Response : {packet}");
    ...
    public void OnToggleNotificationTap()
    {
        ...
        EventManager.Instance.EnableNotification(!toggleNotificationStatus.isOn, EnableNotificationHandler);
    }
    ...
    public void ChangeLanguage()
    {
        ...
        // UIManager.Instance.DisplayLoader(true);
        EventManager.Instance.updatePlayerLanguage(updatePlayerLanguage);
    }

    private void updatePlayerLanguage(Socket socket, Packet packet, object[] args)
    {
        Debug.Log("updatePlayerLanguage response: " + packet.ToString());
    ...
    public void OnLogoutButtonTap()
    {
        ...
        // UIManager.Instance.DisplayLoader(true);
        EventManager.Instance.Logout(LogoutResponse);
    }

    private void LogoutResponse(Socket socket, Packet packet, object[] args)
    {
        Debug.Log($"LogoutResponse: {packet}");
        UIManager.Instance.DisplayLoader(false);
    ...
    public void ModifyBlockMySelfDays(bool incrementAction)
    {
        ...
        // UIManager.Instance.DisplayLoader(true);
        EventManager.Instance.BlockMySelf(BlockingDays, BlockMySelfProcess);
    }

    public void ModifyMonthlyUsageLimit()
    {
        /*if (incrementAction)
            monthlyUsageLimitValue += UIManager.Instance.gameAssetData.monthlyLimitData.incrementValue;
        else
            monthlyUsageLimitValue -= UIManager.Instance.gameAssetData.monthlyLimitData.incrementValue;*/
        ...
        if (monthlyUsageLimitValue < UIManager.Instance.gameAssetData.monthlyLimitData.minMonthlyUsageLimit)
        {
            monthlyUsageLimitValue = UIManager.Instance.gameAssetData.monthlyLimitData.maxMonthlyUsageLimit;
        }
        else if (monthlyUsageLimitValue > UIManager.Instance.gameAssetData.monthlyLimitData.maxMonthlyUsageLimit)
        {
            monthlyUsageLimitValue = UIManager.Instance.gameAssetData.monthlyLimitData.minMonthlyUsageLimit;
        }

        //txtMonthlyUsageLimit.text = monthlyUsageLimitValue.ToString() + " " + Constants.StringClass.currencySymbol;
        // UIManager.Instance.DisplayLoader(true);
        EventManager.Instance.SetLimit(monthlyUsageLimitValue, SetLimitDataProcress);
    }

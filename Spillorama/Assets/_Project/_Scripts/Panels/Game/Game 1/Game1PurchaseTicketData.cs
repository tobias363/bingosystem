...
public class Game1PurchaseTicketData : MonoBehaviour
{
    #region Variables
    ...
    public void Update_Remaining_Tickets(int direction)
    {
        ...
        if (Utility.Instance.IsSplitScreenSupported)
            UIManager.Instance.splitScreenGameManager.game1Panel.Check_Max_Tickets();
        else
            UIManager.Instance.game1Panel.Check_Max_Tickets();
    }

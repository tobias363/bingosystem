...
public class ProfilePanel : MonoBehaviour
{
    ...
    public void OnChangePasswordButtonTap()
    {
        if (ValidateChangePassword())
        {
            ...
            OnChangePassword_Spillorama(oldPassword, newPassword);
        }
    }

    private void OnChangePassword_Spillorama(string oldPassword, string newPassword)
    {
        if (SpilloramaApiClient.Instance == null) return;

        SpilloramaApiClient.Instance.ChangePassword(oldPassword, newPassword,
            (object _) =>
            {
                Debug.Log("[ProfilePanel] ChangePassword success");
                UIManager.Instance.messagePopup.DisplayMessagePopup("Passord endret");
            ...
            {
                Debug.LogWarning($"[ProfilePanel] ChangePassword failed: {code} — {message}");
    ...
    public void OnDeleteAccountButtonTap()
    {
        ...
        UIManager.Instance.messagePopup.DisplayConfirmationPopup(
            LocalizationManager.GetTranslation(Constants.LanguageKey.DeleteConfirmationMessage), (result) =>
            {
                if (result)
                {
                    DeleteAccount_Spillorama();
                }
            });
    }

    private void DeleteAccount_Spillorama()
    {
        if (SpilloramaApiClient.Instance == null) return;

        SpilloramaApiClient.Instance.DeleteAccount(
            (SpilloramaDeletedPayload _) =>
            {
                Debug.Log("[ProfilePanel] DeleteAccount success");
                this.Close();
            ...
            (string code, string message) =>
            {
                Debug.LogWarning($"[ProfilePanel] DeleteAccount failed: {code} — {message}");
                UIManager.Instance.messagePopup.DisplayMessagePopup(message);
            }
        );
    }
    ...
    public void VerifyUser_BankIDTap()
    {
        ...
        SpilloramaApiClient.Instance.InitBankId(
            (SpilloramaBankIdInitResponse result) =>
            {
                ...
                {
                    UIManager.Instance.messagePopup.DisplayMessagePopup(result.message ?? "BankID er ikke konfigurert.");
                    return;
                }

                if (string.IsNullOrEmpty(result.authUrl))
                {
                    UIManager.Instance.messagePopup.DisplayMessagePopup("Kunne ikke starte BankID-verifisering.");
                    return;
                }

                Debug.Log($"[ProfilePanel] BankID init: opening authUrl");
            ...
            (string code, string message) =>
            {
                BankId_Btn.interactable = true;
                BankId_Reverification_Btn.interactable = true;
                Debug.LogWarning($"[ProfilePanel] BankID init failed: {code} — {message}");
                UIManager.Instance.messagePopup.DisplayMessagePopup(message);
            }
        );
    }
    ...
    {
        // Image upload not supported via Spillorama REST yet — use BankID verification instead
        UIManager.Instance.messagePopup.DisplayMessagePopup("Bildeopplasting er ikke tilgjengelig. Bruk BankID-verifisering.");
    }
    ...
    {
        CallPlayerProfileEvents_Spillorama();
    }

    private void CallPlayerProfileEvents_Spillorama()
    {
        if (SpilloramaApiClient.Instance == null) return;

        SpilloramaApiClient.Instance.GetProfile(
            (SpilloramaUserPayload user) =>
            {
                playerData = new PlayerProfile
                {
                    playerId = user.id,
                    email = user.email,
                    ...
                    customerNumber = 0,
                    hall = new HallData(),
                    isVerifiedByBankID = user.kycStatus == "VERIFIED",
                    isVerifiedByHall = user.kycStatus == "VERIFIED",
            ...
            (string code, string message) =>
            {
                Debug.LogWarning($"[ProfilePanel] GetProfile failed: {code} — {message}");
                UIManager.Instance.messagePopup.DisplayMessagePopup(message);
            }
        );
    }
    ...
    {
        CallUpdateProfileData_Spillorama();
    }

    private void CallUpdateProfileData_Spillorama()
    {
        if (SpilloramaApiClient.Instance == null) return;

        string displayName = inputNickname.text;
        string email = inputEmail.text;
        string phone = inputMobileNumber.text;

        SpilloramaApiClient.Instance.UpdateProfile(displayName, email, phone,
            (SpilloramaUserPayload updated) =>
            {
                Debug.Log($"[ProfilePanel] UpdateProfile success: {updated.displayName}");
                UIManager.Instance.messagePopup.DisplayMessagePopup("Profil oppdatert");
                CallPlayerProfileEvents_Spillorama();
            },
            (string code, string message) =>
            {
                Debug.LogWarning($"[ProfilePanel] UpdateProfile failed: {code} — {message}");
                UIManager.Instance.messagePopup.DisplayMessagePopup(message);
            }
        );
    }

...
async function makeEngineWithRoom() {
...
function prioritizeDrawNumbers(engine, roomCode, preferredNumbers) {
...
test("startGame rejects ticketsPerPlayer below 1", async () => {
    ...
    await assert.rejects(async () => engine.startGame({ roomCode, actorPlayerId: hostPlayerId, ticketsPerPlayer: 0 }), (error) => error instanceof DomainError && error.code === "INVALID_TICKETS_PER_PLAYER");
});
test("startGame rejects ticketsPerPlayer above 5", async () => {
    const { engine, roomCode, hostPlayerId } = await makeEngineWithRoom();
    await assert.rejects(async () => engine.startGame({ roomCode, actorPlayerId: hostPlayerId, ticketsPerPlayer: 6 }), (error) => error instanceof DomainError && error.code === "INVALID_TICKETS_PER_PLAYER");
});
...
test("startGame rejects payoutPercent outside 0-100", async () => {
    const { engine, roomCode, hostPlayerId } = await makeEngineWithRoom();
    await assert.rejects(async () => engine.startGame({ roomCode, actorPlayerId: hostPlayerId, payoutPercent: -1 }), (error) => error instanceof DomainError && error.code === "INVALID_PAYOUT_PERCENT");
    await assert.rejects(async () => engine.startGame({ roomCode, actorPlayerId: hostPlayerId, payoutPercent: 101 }), (error) => error instanceof DomainError && error.code === "INVALID_PAYOUT_PERCENT");
});
...
test("round ends automatically when max draws is reached", async () => {
    ...
    await assert.rejects(async () => limitedEngine.drawNextNumber({
        roomCode: limitedRoomCode,
        actorPlayerId: limitedHostPlayerId
    }), (error) => error instanceof DomainError && error.code === "GAME_NOT_RUNNING");
});
test("joinRoom rejects duplicate wallet in same room", async () => {
    ...
    await assert.rejects(async () => engine.joinRoom({
        roomCode,
        hallId: "hall-1",
        playerName: "Host Duplicate",
        walletId: "wallet-host"
    }), (error) => error instanceof DomainError && error.code === "PLAYER_ALREADY_IN_ROOM");
});
test("createRoom rejects wallet already in running game", async () => {
    ...
    await assert.rejects(async () => engine.createRoom({
        ...
        walletId: "wallet-guest"
    }), (error) => error instanceof DomainError && error.code === "PLAYER_ALREADY_IN_RUNNING_GAME");
});
...
test("timed pause cannot be cancelled early and blocks gameplay actions", async () => {
    ...
    await withFakeNow(20_000, async () => {
        ...
        assert.throws(() => engine.clearTimedPause("wallet-paused"), (error) => error instanceof DomainError && error.code === "TIMED_PAUSE_LOCKED");
    });
...
test("self-exclusion cannot be lifted before one year and is checked on gameplay actions", async () => {
    ...
    await withFakeNow(2_000, async () => {
        ...
        assert.throws(() => engine.clearSelfExclusion("wallet-host"), (error) => error instanceof DomainError && error.code === "SELF_EXCLUSION_LOCKED");
    });
...
test("extra draw attempts are rejected and audited", async () => {
    const { engine, roomCode, hostPlayerId } = await makeEngineWithRoom();
    await assert.throws(() => engine.rejectExtraDrawPurchase({
        source: "SOCKET",
        roomCode,
        playerId: hostPlayerId,
        metadata: {
    ...
    }), (error) => error instanceof DomainError && error.code === "EXTRA_DRAW_NOT_ALLOWED");
...
test("prize policy supports hall/link effective dates and extra-prize daily cap", async () => {
    ...
    await withFakeNow(nextDay + 6_000, async () => {
        await assert.rejects(async () => engine.awardExtraPrize({
            walletId: "wallet-prize-2",
            hallId: "hall-2",
            linkId: "hall-2",
            amount: 1500,
            reason: "campaign"
        }), (error) => error instanceof DomainError && error.code === "EXTRA_PRIZE_DAILY_LIMIT_EXCEEDED");
    });
});
...
test("daily report separates hall/game/channel ledgers and exports csv", async () => {
    ...
    const hallMain = report.rows.find((row) => row.hallId === "hall-1" && row.gameType === "MAIN_GAME" && row.channel === "HALL");
    const internetMain = report.rows.find((row) => row.hallId === "hall-1" && row.gameType === "MAIN_GAME" && row.channel === "INTERNET");
    const databingo = report.rows.find((row) => row.hallId === "hall-1" && row.gameType === "DATABINGO" && row.channel === "INTERNET");

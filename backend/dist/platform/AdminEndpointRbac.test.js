...
const ALL_ROLES = ["ADMIN", "HALL_OPERATOR", "SUPPORT", "PLAYER"];
test("admin-endepunkter følger RBAC-policy", () => {
    for (const policyCase of ENDPOINT_POLICY_CASES) {
        for (const role of ALL_ROLES) {
            ...
            assert.equal(canAccessAdminPermission(role, policyCase.permission), expected, `${role} mismatch for ${policyCase.endpoint} (${policyCase.permission})`);
        }
    }
});

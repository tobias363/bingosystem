...
import { ADMIN_ACCESS_POLICY, assertAdminPermission, canAccessAdminPermission } from "./AdminAccessPolicy.js";
const NON_ADMIN_ROLES = ["HALL_OPERATOR", "SUPPORT", "PLAYER"];
...
test("sensitive write permissions are restricted to admin", () => {
    ...
    for (const permission of adminOnlyPermissions) {
        ...
        for (const role of NON_ADMIN_ROLES) {
            assert.equal(canAccessAdminPermission(role, permission), false, `${role} should not access ${permission}`);
        }
    }
});
...
test("policy entries only include known roles", () => {
    const knownRoles = ["ADMIN", "HALL_OPERATOR", "SUPPORT", "PLAYER"];
    for (const permission of Object.keys(ADMIN_ACCESS_POLICY)) {

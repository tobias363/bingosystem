...
const ADMIN_ACCESS_POLICY_DEFINITION = {
    ...
    INTEGRATION_READ: ["ADMIN", "SUPPORT"]
};
export const ADMIN_ACCESS_POLICY = ADMIN_ACCESS_POLICY_DEFINITION;
export function canAccessAdminPermission(role, permission) {
    return ADMIN_ACCESS_POLICY[permission].includes(role);
}
export function listAdminPermissionsForRole(role) {
    return Object.keys(ADMIN_ACCESS_POLICY).filter((permission) => canAccessAdminPermission(role, permission));
}
export function getAdminPermissionMap(role) {
    const map = {};
    for (const permission of Object.keys(ADMIN_ACCESS_POLICY)) {
        map[permission] = canAccessAdminPermission(role, permission);
    }
    return map;
}
export function assertAdminPermission(role, permission, message) {

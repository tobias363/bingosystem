...
function parseNonNegativeNumberFromEnv(name, fallback) {
...
function parsePositiveNumberFromEnv(name, fallback) {
    const raw = process.env[name];
    if (raw === undefined || raw === null || raw.trim() === "") {
        return fallback;
    }
...
export function createWalletAdapter(projectDir) {
    ...
    if (provider === "postgres" || provider === "pg") {
        ...
        if (!connectionString) {
            throw new WalletError("INVALID_WALLET_CONFIG", "WALLET_PG_CONNECTION_STRING må settes når WALLET_PROVIDER=postgres.");
        }
    ...
    throw new WalletError("INVALID_WALLET_CONFIG", `Ukjent WALLET_PROVIDER "${provider}". Bruk "file", "http", "postgres" eller "external".`);
}

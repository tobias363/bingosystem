...
describe("ExternalWalletAdapter", () => {
    ...
    it("handles INSUFFICIENT_FUNDS from provider", async () => {
        ...
        await assert.rejects(() => adapter.debit("player-1", 10000, "Buy-in ROOM1"), (err) => {
    ...
    it("rejects player-to-player transfer", async () => {
        await assert.rejects(() => adapter.transfer("player-1", "player-2", 100, "P2P"), (err) => {
...
describe("WebhookService", () => {
    ...
    it("sends compliance event", async () => {
        ...
        const payload = service.buildCompliancePayload("compliance.lossLimitReached", "player-1", { dailyLoss: 900, limit: 900 });

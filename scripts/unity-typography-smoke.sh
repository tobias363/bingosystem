...
UNITY_BIN="${UNITY_BIN:-}"
LOG_FILE="${UNITY_TYPOGRAPHY_SMOKE_LOG:-/tmp/unity_typography_smoke.log}"
...
echo "Running gameplay typography smoke test..."
"$UNITY_BIN" \
  -batchmode \
  -nographics \
  -projectPath "$PROJECT_PATH" \
  -executeMethod CandyGameplayTypographySmoke.RunFromCommandLine \
  -logFile "$LOG_FILE"

if ! rg -n "\[TypographySmoke\] RESULT status=PASS" "$LOG_FILE" >/dev/null; then
  echo "Gameplay typography smoke test did not report PASS. Check log: $LOG_FILE" >&2
  tail -n 120 "$LOG_FILE" >&2 || true
  exit 1
fi

echo "Gameplay typography smoke test passed."

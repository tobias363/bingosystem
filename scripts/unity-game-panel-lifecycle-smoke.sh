...
UNITY_BIN="${UNITY_BIN:-}"
LOG_FILE="${UNITY_GAME_PANEL_LIFECYCLE_LOG:-/tmp/unity_game_panel_lifecycle_smoke.log}"
...
echo "Running game panel lifecycle smoke test..."
"$UNITY_BIN" \
  -batchmode \
  -nographics \
  -quit \
  -projectPath "$PROJECT_PATH" \
  -executeMethod GamePanelLifecycleSmokeTests.RunGamePanelLifecycleSmokeTest \
  -logFile "$LOG_FILE"

if ! rg -n "\[GamePanelLifecycleSmoke\] PASS" "$LOG_FILE" >/dev/null; then
  echo "Game panel lifecycle smoke test did not report PASS. Check log: $LOG_FILE" >&2
  tail -n 80 "$LOG_FILE" >&2 || true
  exit 1
fi

echo "Game panel lifecycle smoke test passed."

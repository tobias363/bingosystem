...
UNITY_BIN="${UNITY_BIN:-}"
LOG_FILE="${UNITY_GAME_FLOW_CONTRACT_LOG:-/tmp/unity_game_flow_contract_smoke.log}"
...
echo "Running game flow contract smoke test..."
"$UNITY_BIN" \
  -batchmode \
  -nographics \
  -quit \
  -projectPath "$PROJECT_PATH" \
  -executeMethod GameFlowContractSmokeTests.RunGameFlowContractSmokeTest \
  -logFile "$LOG_FILE"

if ! rg -n "\[GameFlowContractSmoke\] PASS" "$LOG_FILE" >/dev/null; then
  echo "Game flow contract smoke test did not report PASS. Check log: $LOG_FILE" >&2
  tail -n 80 "$LOG_FILE" >&2 || true
  exit 1
fi

echo "Game flow contract smoke test passed."

...
UNITY_BIN="${UNITY_BIN:-}"
LOG_FILE="${UNITY_GAME_PANEL_SMOKE_LOG:-/tmp/unity_game_panel_smoke.log}"
...
echo "Running game panel wiring smoke test..."
"$UNITY_BIN" \
  -batchmode \
  -nographics \
  -quit \
  -projectPath "$PROJECT_PATH" \
  -executeMethod GamePanelWiringSmokeTests.RunGamePanelSceneWiringSmokeTest \
  -logFile "$LOG_FILE"

if ! rg -n "\[GamePanelWiringSmoke\] PASS" "$LOG_FILE" >/dev/null; then
  echo "Game panel wiring smoke test did not report PASS. Check log: $LOG_FILE" >&2
  tail -n 80 "$LOG_FILE" >&2 || true
  exit 1
fi

echo "Game panel wiring smoke test passed."

...
export interface ClaimRecord {
  ...
  /**
   * CRIT-6 (SPILL1_CASINO_GRADE_REVIEW_2026-04-26): status på post-transfer
   * audit-trail. `complete` = alle 5 stegene (compliance loss, ledger,
   * payout-audit, checkpoint, persist) gikk gjennom. `degraded` = minst
   * ett steg feilet og ble lagt på recovery-queue (eller logget for
   * manuell replay om porten ikke er konfigurert).
   *
   * Settes kun for vellykkede claim-payouts (valid + payoutAmount > 0).
   * Tjener både som operativ flagg og som test-anker for å verifisere
   * at recovery-pathen faktisk ble utløst ved partial-failure.
   */
  auditTrailStatus?: "complete" | "degraded";
  createdAt: string;
}
